      ...
      <Tabs defaultValue="generate" className="space-y-6">
        ...
        <TabsContent value="recurrence" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <div>
                <Label htmlFor="assunto-recorrencia">Assunto Específico</Label>
                <Input 
                  id="assunto-recorrencia" 
                  placeholder="Ex: rotina matinal, dicas de economia"
              ...
              <div>
                <Label htmlFor="periodoCiclo">Duração do Ciclo (dias): {periodoCiclo}</Label>
                <input
                  type="range"
                  id="periodoCiclo"
                  min="7"
          ...
          <div className="flex justify-end space-x-4">
            <Button 
              ...
              disabled={loadingRecorrencia || !nicho || !formatoCanal}
            >
              {loadingRecorrencia ? "Analisando..." : "Analisar Estruturas"}
            </Button>
          ...
          {estruturasRecorrencia.length > 0 && (
            <div className="mt-6">
              <h3 className="text-lg font-medium mb-2">Estruturas de Recorrência ({estruturasRecorrencia.length})</h3>
              <div className="space-y-4">
                {estruturasRecorrencia.slice(0, 5).map((estrutura, index) => (
                  ...
                  >
                    ...
                    <p className="text-sm">{estrutura.descricao}</p>
                    <div className="mt-2 text-xs text-muted-foreground">
                      <span>Exemplo: "{estrutura.exemplo_titulo}"</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {gatilhosRecorrencia.length > 0 && (
            <div className="mt-6">
              <h3 className="text-lg font-medium mb-2">Gatilhos de Recorrência ({gatilhosRecorrencia.length})</h3>
                ...
                {gatilhosRecorrencia.map((gatilho, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <h4 className="font-medium">{gatilho.tipo_gatilho}</h4>
                      <Badge variant="outline">
                        {gatilho.eficacia}
                      </Badge>
                    </div>
                    <p className="text-xs text-muted-foreground">{gatilho.descricao}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-8 flex justify-end space-x-4">
                ...
                <Button 
                  ...
                  disabled={titulosRecorrencia.length === 0 || !frequenciaPublicacao || periodoCiclo === 0}
                >
                  Planejar Ciclo de Conteúdo
                </Button>
              </div>
            </div>
          )}
              ...
              <div className="space-y-2">
                {titulosRecorrencia.map((titulo, index) => (
                  <div 
                    ...
                    className="p-3 rounded-md bg-muted/50 hover:bg-muted/80 transition-colors flex justify-between items-center"
                  >
                    <span>{titulo}</span>
                    <Button 
                      variant="ghost" 
